...
{
    private final String serverLocation = "https://raw.github.com/TheCricket/VanillaPlus/master/Capes.txt";
    ...
    public void onPreRenderSpecials(RenderPlayerEvent.Specials.Pre event)
    {
        if (event.entityPlayer instanceof AbstractClientPlayer)
        {
            ...
            {
                String cloakURL = capes.get(event.entityPlayer.getDisplayName());
                ...
                ReflectionHelper.setPrivateValue(ThreadDownloadImageData.class, abstractClientPlayer.getTextureCape(), false, new String[]{"textureUploaded", "textureUploaded"});
        ...
        try
        {
            ...
            while ((str = br.readLine()) != null)
            {
                if (!str.startsWith("--") && !str.isEmpty())
                {
                    ...
                    {
                        System.err.println("[VanillaPlus] [Capes.txt] Syntax error on line " + linetracker + ": " + str);
                    }
                }
                linetracker++;
            }
    ...
    private class CloakThread implements Runnable
    {
        ...
        public void run()
        {
            try
            {
                ...
                ReflectionHelper.setPrivateValue(ThreadDownloadImageData.class, abstractClientPlayer.getTextureCape(), bo, new String[] { "bufferedImage", "bufferedImage" });
            }
            catch (MalformedURLException e)
            {
                e.printStackTrace();
    ...
    private class CloakPreload implements Runnable
    {
        ...
        public void run()
        {
            try
            {
                TEST_GRAPHICS.drawImage(new ImageIcon(new URL(capeURL)).getImage(), 0, 0, null);
            }
            catch (MalformedURLException e)
            {
                e.printStackTrace();
