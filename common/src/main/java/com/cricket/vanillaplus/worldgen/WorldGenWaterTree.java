...
public class WorldGenWaterTree extends WorldGenerator{
	...
	public WorldGenWaterTree(boolean doNotify){
		super(doNotify);
	}
	
	public WorldGenWaterTree(){
		super();
	}
	...
	public boolean growTree(World world, Random rand, int x, int y, int z){
		...
		for(int yy = y; yy < y + trunkHeight; ++yy){
			world.setBlock(x, yy, z, Registry.BlockWaterInfusedLog);
		}
		...
		for(int a = 0, xx, zz; a < 4; ++a){
			...
			for(int branch = 0; branch < branchSizes[a]; ++branch){
				...
				world.setBlock(xx, y + trunkHeight - (branchSizes[a] == 1 ? 1 : 2 - branch), zz, Registry.BlockWaterInfusedLog, 0 + a <= 1 ? 4 : 8, 3);
		...
		for(int yy = y + trunkHeight + 1; yy <= y + trunkHeight + 2; yy++){
			for(int xx = x - 4; xx <= x + 4; xx++){
				for(int zz = z - 4; zz <= z + 4; zz++){
					if(world.getBlock(xx - 1, yy - 1, zz) == Registry.BlockWaterInfusedLeaves && world.getBlock(xx + 1, yy - 1, zz) == Registry.BlockWaterInfusedLeaves && world.getBlock(xx, yy - 1, zz - 1) == Registry.BlockWaterInfusedLeaves && world.getBlock(xx, yy - 1, zz + 1) == Registry.BlockWaterInfusedLeaves){
						world.setBlock(xx, yy, zz, Registry.BlockWaterInfusedLeaves, 0, 3);
					}
				}
			}
		}
		...
		for(int xx = x - 2, xDiff, zDiff; xx <= x + 2; ++xx){
			for(int zz = z - 2; zz <= z + 2; ++zz){
				...
				if(((xDiff <= 1 && zDiff <= 1) || (xDiff == 2 && zDiff <= sideSize) || (xDiff <= sideSize && zDiff == 2)) && world.isAirBlock(xx, y, zz)){
					world.setBlock(xx, y, zz, Registry.BlockWaterInfusedLeaves, 0 , 3);
				}
			}
		}
	}
