...
public class VPGenRavines extends VPMapGenBase{
	...
	protected void generate(World world, int x, int z, int x1, int z1, Block[] blocks, byte[] meta){
		if(this.rand.nextInt(50) == 0){
			...
			double d2 = z * 16 + this.rand.nextInt(this.rand.nextInt(16));
			float f = this.rand.nextFloat() * 3.141593F * 2.0F;
			float f1 = (this.rand.nextFloat() - 0.5F) * 0.25F;
			float f2 = (this.rand.nextFloat() * 2.0F + this.rand.nextFloat()) * 2.0F;
			
			this.generate(this.rand.nextLong(), x, z, blocks, meta, d0, d1, d2, f2, f, f1);
		}
	}
	
	protected void generate(long seed, int x, int z, Block[] blocks, byte[] meta, double x1, double y1, double z1, float rotation, float r2, float r3){
		Random rand = new Random();
		double d4 = x * 16 + 8;
		double d5 = z * 16 + 8;
		float f3 = 0.0F;
		float f4 = 0.0F;
		int j1 = 112 - rand.nextInt(28);
		boolean flag1 = false;
		float f5 = 1.0F;
		...
		for(int c = 0; c < j1; c++){
			double d12 = 1.5D + MathHelper.sin(c *3.141593F / j1) * rotation * 1.0F;
			double d6 = d12 * 3D;
			d12 *= (rand.nextFloat() * 0.25D + 0.75D);
			d6 *= (rand.nextFloat() * 0.25D + 0.75D);
			float f6 = MathHelper.cos(r3);
			float f7 = MathHelper.sin(r3);
			x1 += MathHelper.cos(r3) * f6;
			y1 += f7;
			z1 += MathHelper.sin(r3) * f6;
			r3 *= 0.7f;
			r3 *= f4 * 0.05F;
			r3 *= f3 * 0.05F;
			f4 *= 0.8F;
					...
					for(int j3 = k4; j3 < j2; j3++){
						...
						if(d13 * d13 + d14 * d14 < 1.0D){
							for(int l3 = i2 - 1; l3 >= j4; l3--){
								...
								if((d13 * d13 + d14 * d14) * this.floats[13] + d11 * d11 / 6.0D < 1.0D){
